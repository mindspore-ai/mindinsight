...
class OnesMapper(AtenToMindSporeMapper):
    """Ones mapper."""
    ...
    def _convert_trained_weights(**kwargs):
        ...
        args_name_list = OnesMapper.get_args_name_list(**kwargs, args_name=args_name)
    ...
    def _generate_snippet_template(**kwargs):
        ...
        args_name = ["input", "shape", "dtype", "unused", "unused", "unused"]
        raw_args_name_list = OnesMapper.get_args_name_list(params=raw_params,
                                                           args_name=args_name,
                                                           return_raw=True)
        inputs, args, group_inputs = OnesMapper._params_parser(raw_params, args_name, trainable_params)
        ...
        init_template_list = [f"self.{{{variable_slot}}}_{arg_name} = {{{arg_name}}}" for arg_name in args
                              if arg_name != "unused"]
        parameters_declared = OnesMapper.generate_parameters_declared(variable_slot, init_template_list, args,
                                                                      trainable_params)
        diff = len(inputs) - len(raw_args_name_list)
        if diff > 0:
            construct_template = f"opt_{{{variable_slot}}} = ms_np.ones" \
