...
class CopyMapper(AtenToMindSporeMapper):
    """Copy mapper."""
    ...
    def _convert_trained_weights(**kwargs):
        ...
        args_name = ["input"]
        args_name_list = CopyMapper.get_args_name_list(**kwargs, args_name=args_name)
    ...
    def _generate_snippet_template(**kwargs):
        ...
        args_name = ["input"]
        inputs, args, group_inputs = CopyMapper._params_parser(raw_params, args_name, trainable_params)

        init_template_list = list()
        parameters_declared = CopyMapper.generate_parameters_declared(variable_slot, init_template_list,
                                                                      args, trainable_params)
        construct_template_list = [f"opt_{{{variable_slot}}} = {inputs[0]}.copy()"]
        if CopyMapper.IN_PLACE:
