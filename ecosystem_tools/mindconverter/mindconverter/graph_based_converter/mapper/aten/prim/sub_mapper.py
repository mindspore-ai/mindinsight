...
class SubMapper(AtenToMindSporeMapper):
    """Sub mapper."""
    ...
    def _convert_trained_weights(**kwargs):
        ...
        args_name_list = ["input_0", "input_1", "alpha"]
    ...
    def _generate_snippet_template(**kwargs):
        ...
        args_name_list = ["input_0", "input_1", "alpha"]
        inputs, args, group_inputs = SubMapper._params_parser(raw_params, args_name_list, trainable_params)
        ...
        if op == "P.RSub":
            construct_template = f"opt_{{{variable_slot}}} = {inputs[1]} - {inputs[0]} * {inputs[2]}"
        else:
            construct_template = f"opt_{{{variable_slot}}} = {inputs[0]} - {inputs[1]} * {inputs[2]}"
