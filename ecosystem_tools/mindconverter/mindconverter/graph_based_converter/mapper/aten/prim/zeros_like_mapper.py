...
class ZerosLikeMapper(AtenToMindSporeMapper):
    """ZerosLike mapper."""
    ...
    def _convert_trained_weights(**kwargs):
        ...
        args_name_list_mapper = {
            3: ["input", "unused", "unused"],
            6: ["input", "dtype", "unused", "unused", "unused", "unused"]
        }
        args_name_list = ZerosLikeMapper.get_args_name_list(**kwargs, args_name=args_name_list_mapper)
    ...
    def _generate_snippet_template(**kwargs):
        ...
        inputs, args, group_inputs = ZerosLikeMapper._params_parser(raw_params, args_name_list_mapper, trainable_params)
        ...
        for name, trainable_param in trainable_params.copy().items():
            ...
            if ZerosLikeMapper.is_tensor(value):
        ...
        construct_template = f"opt_{{{variable_slot}}} = ms_np.{op}" \
                             f"({inputs[0]}, dtype=self.{{{variable_slot}}}_dtype)"
