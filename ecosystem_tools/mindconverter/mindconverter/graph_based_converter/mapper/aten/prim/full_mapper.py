...
class FullMapper(AtenToMindSporeMapper):
    """Full mapper."""
    ...
    def _convert_trained_weights(**kwargs):
        ...
        args_name_list_mapper = {
            3: ["shape", "value", "unused"],
            4: ["shape", "value", "unused", "unused"],
            6: ["shape", "value", "dtype", "unused", "unused", "unused"],
            7: ["shape", "value", "unused", "dtype", "unused", "unused", "unused"]
        }
        args_name_list = FullMapper.get_args_name_list(**kwargs, args_name=args_name_list_mapper)
    ...
    def _generate_snippet_template(**kwargs):
        ...
        raw_args_name_list = FullMapper.get_args_name_list(params=raw_params,
                                                           args_name=args_name_list_mapper,
                                                           return_raw=True)
        inputs, args, group_inputs = FullMapper._params_parser(raw_params, args_name_list_mapper, trainable_params)
        ...
        init_template_list = [f"self.{{{variable_slot}}}_{arg_name} = {{{arg_name}}}" for arg_name in args
                              if arg_name not in ["unused", "value"]]
        if args.get("value") == float("-inf"):
            init_val = f"self.{{{variable_slot}}}_value = float('{{value}}')"
        else:
            init_val = f"self.{{{variable_slot}}}_value = {{value}}"
        init_template_list.append(init_val)
        parameters_declared = dict()
        for name, trainable_param in trainable_params.copy().items():
            value = trainable_param["data"]
            if FullMapper.is_tensor(value):
