...
class MeanMapper(AtenToMindSporeMapper):
    """Mean mapper."""
    ...
    def _convert_trained_weights(**kwargs):
        ...
        args_name_list_mapper = {
            2: ["input", "dtype"],
            4: ["input", "axis", "keep_dims", "dtype"]
        }
        args_name_list = MeanMapper.get_args_name_list(**kwargs, args_name=args_name_list_mapper)
    ...
    def _generate_snippet_template(**kwargs):
        ...
        if args.get("axis", "axis") == "axis":
            construct_template = f"opt_{{{variable_slot}}} = ms_np.mean" \
                                 f"({inputs[0]}, dtype={inputs[1]})"
        else:
            init_keep_dims = f"self.{{{variable_slot}}}_keep_dims = bool({{keep_dims}})"
            init_template_list.append(init_keep_dims)
            construct_template = f"opt_{{{variable_slot}}} = ms_np.mean" \
                                 f"({inputs[0]}, {inputs[1]}, " \
                                 f"{inputs[2]}, {inputs[3]})"
