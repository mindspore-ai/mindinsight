...
class FillMapper(AtenToMindSporeMapper):
    """Fill mapper."""
    ...
    def _operation_name_in_ms(*args, **kwargs):
        ...
        if op_name.endswith("_"):
            FillMapper.IN_PLACE = True
        return "fill"

    @staticmethod
    def _convert_trained_weights(**kwargs):
        weights = kwargs.get("weights", list())
        args_name = ["input", "value"]
        args_name_list = FillMapper.get_args_name_list(**kwargs, args_name=args_name)
    ...
    def _generate_snippet_template(**kwargs):
        ...
        variable_slot = "var_0"
        args_name = ["input", "value"]
        inputs, args, group_inputs = FillMapper._params_parser(raw_params, args_name, trainable_params)

        init_template_list = list()
        if args.get("value") == float("-inf"):
            init_value = f"self.{{{variable_slot}}}_value = float('{{value}}')"
        else:
            init_value = f"self.{{{variable_slot}}}_value = {{value}}"
        init_template_list.append(init_value)
        parameters_declared = dict()
        for name, trainable_param in trainable_params.copy().items():
            value = trainable_param["data"]
            if FillMapper.is_tensor(value):
        ...
        construct_template_list = [f"opt_{{{variable_slot}}} = {inputs[0]}.{op}({inputs[-1]})"]
        if FillMapper.IN_PLACE:
            construct_template_list.append(f"{inputs[0]} = opt_{{{variable_slot}}}")
