...
class BatchMatMulMapper(AtenToMindSporeMapper):
    """BatchMatMul mapper."""

    @staticmethod
    def _operation_name_in_ms(*args, **kwargs):
        return "P.BatchMatMul"
    ...
    def _generate_snippet_template(**kwargs):
        ...
        variable_slot = "var_0"
        args_name_list = ["input0", "input1"]
        inputs, args, group_inputs = BatchMatMulMapper._params_parser(raw_params, args_name_list, trainable_params)
        ...
        for name, trainable_param in trainable_params.copy().items():
            ...
            if BatchMatMulMapper.is_tensor(value):
        ...
        construct_template = f"opt_{{{variable_slot}}} = self.{{{variable_slot}}}({', '.join(inputs)})"
