...
class MaskedFillMapper(AtenToMindSporeMapper):
    ...
    def _convert_trained_weights(**kwargs):
        ...
        args_name = ["input", "mask", "val"]
        args_name_list = MaskedFillMapper.get_args_name_list(**kwargs, args_name=args_name)
    ...
    def _generate_snippet_template(**kwargs):
        ...
        variable_slot = "var_0"
        args_name = ["input", "mask", "val"]
        inputs, args, group_inputs = MaskedFillMapper._params_parser(raw_params, args_name, trainable_params)

        init_template_list = list()
        init_val = f"self.{{{variable_slot}}}_val = float('{{val}}')"
