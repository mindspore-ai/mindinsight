...
class CumSumMapper(AtenToMindSporeMapper):
    """CumSum mapper."""
    ...
    def _convert_trained_weights(**kwargs):
        ...
        args_name = ["input", "axis", "dtype"]
        args_name_list = CumSumMapper.get_args_name_list(**kwargs, args_name=args_name)
    ...
    def _generate_snippet_template(**kwargs):
        ...
        variable_slot = "var_0"
        args_name = ["input", "axis", "dtype"]
        inputs, args, group_inputs = CumSumMapper._params_parser(raw_params=raw_params, args_name=args_name,
                                                                 trainable_params=trainable_params)

        init_template_list = [f"self.{{{variable_slot}}}_{arg_name} = {{{arg_name}}}" for arg_name in args if
                              arg_name != "dtype"]
        parameters_declared = dict()
        for name, trainable_param in trainable_params.copy().items():
            value = trainable_param["data"]
            if CumSumMapper.is_tensor(value):
