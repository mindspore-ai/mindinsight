...
class SqueezeMapper(AtenToMindSporeMapper):
    """Squeeze mapper."""
    ...
    def _generate_snippet_template(**kwargs):
        ...
        args_name_list = ["input", "axis"]
        inputs, args, group_inputs = SqueezeMapper._params_parser(raw_params, args_name_list, trainable_params)

        init_template_list = [f"self.{{{variable_slot}}} = {op}(axis={{axis}})"]
        parameters_declared = dict()
        for name, trainable_param in trainable_params.copy().items():
            value = trainable_param["data"]
            if SqueezeMapper.is_tensor(value):
