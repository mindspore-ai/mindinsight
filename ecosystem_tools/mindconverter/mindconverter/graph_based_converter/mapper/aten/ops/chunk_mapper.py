...
class SplitMapper(AtenToMindSporeMapper):
    ...
    def _convert_trained_weights(**kwargs):
        ...
        args_name = ["input", "output_num", "axis"]
        args_name_list = SplitMapper.get_args_name_list(**kwargs, args_name=args_name)
    ...
    def _generate_snippet_template(**kwargs):
        ...
        args_name_list = ["input", "output_num", "axis"]
        inputs, args, group_inputs = SplitMapper._params_parser(raw_params, args_name_list, trainable_params)

        init_template_list = [f"self.{{{variable_slot}}} = P.Split(axis={{axis}}, output_num={{output_num}})"]
        parameters_declared = dict()
        for name, trainable_param in trainable_params.copy().items():
            value = trainable_param["data"]
            if SplitMapper.is_tensor(value):
        ...
        output_num = args.get("output_num")
