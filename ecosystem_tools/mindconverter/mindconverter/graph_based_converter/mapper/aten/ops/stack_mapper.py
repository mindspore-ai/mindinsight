...
class StackMapper(AtenToMindSporeMapper):
    """Stack mapper."""
    ...
    def _convert_trained_weights(**kwargs):
        ...
        args_name = ["input", "axis"]
        args_name_list = StackMapper.get_args_name_list(**kwargs, args_name=args_name)
    ...
    def _generate_snippet_template(**kwargs):
        ...
        inputs, args, group_inputs = StackMapper._params_parser(raw_params, args_name_list, trainable_params)
        args["axis"] = 0 if args.get("axis") is None else args.get("axis")
        ...
        for name, trainable_param in trainable_params.copy().items():
            ...
            if StackMapper.is_tensor(value):
        ...
        construct_template = f"opt_{{{variable_slot}}} = self.{{{variable_slot}}}(({' ,'.join(inputs[:-1])}))"
