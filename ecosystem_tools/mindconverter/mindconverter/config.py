...
from mindconverter.common.exceptions import CodeSyntaxError

REQUIRED = 'REQUIRED'
UNREQUIRED = 'UNREQUIRED'
FUNC_MODULE = 'mindconverter.funcs'
    ...
    def parse_args(self, call_name: str, args_str: str):
        ...
        for arg in call_node.args:
            if isinstance(arg, ast.Starred):
                MindConverterLogger.debug(f"Find *{arg.value.id}")
                args['*'] = arg.value.id
            else:
                # remove \n
        ...
        for keyword in call_node.keywords:
            if keyword.arg is None:
                MindConverterLogger.info(f"Find **{keyword.value.id}")
