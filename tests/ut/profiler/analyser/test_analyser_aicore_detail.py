...
COL_NAMES = ['op_name', 'op_type', 'avg_execution_time', 'execution_frequency', 'FLOPs(cube)', 'FLOPS(cube)',
             'FLOPs(vec)', 'FLOPS(vec)', 'full_op_name', 'op_info']
        ...
        for fm_info, detail_info, flops_line in zip(fm_csv_reader, detail_csv_reader, flops_reader):
            ...
            cache.append(
                [fm_info[4], fm_info[5], round(float(detail_info[1]) * 1e3, 3), int(detail_info[2]), float(flops[1]),
                 float(flops[2]), float(flops[4]), float(flops[5]),
