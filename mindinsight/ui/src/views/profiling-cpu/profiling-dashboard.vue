  ...
  <div class="prof-wrap">
    <div class="prof-head">
      ...
      <div class="path-message">
        ...
        <span>{{curDashboardInfo.query.path}}</span>
    ...
    <div class="prof-content">
      <div class="prof-content-left"
        ...
        <div class="collapse-btn"
             :class="[
               collapse ? 'collapse' : '',
               `collapse-btn-${$store.state.themeIndex}`
             ]"
             @click="collapseLeft">
        </div>
      </div>
      <div class="prof-content-right"
        ...
        <div class="tab-container"
             v-show="
               ['/profiling-cpu/performance-dashboard', '/profiling-cpu/resource-dashboard'].includes($route.path)
             ">
...
export default {
  ...
  watch: {},
  mounted() {
    if (this.$route.path === 'resource-dashboard') {
  ...
  methods: {
            ...
            dataKeys.forEach((item) => {
              ...
              ${this.$t(`profiling`)[item].desc}
                ...
                if (item === 'minddata_device_queue_rate') {
                  if (resp.data[item].empty_rate > resp.data[item].empty_warning_threshold) {
                    content = this.$t(`profiling.${item}.empty_rate`, resp.data[item]);
                  } else {
                    content = this.$t(`profiling.${item}.empty_warning_threshold`, resp.data[item]);
                  }
                } else {
                  content = `${this.$t(`profiling`)[item].desc}`.replace(`{n1}`, resp.data[item][0]);
                }
              ...
              <div class="helper-content-style">${content}</div>`;
                helperDiv.appendChild(divDom);
              } else if (item === 'device_queue_warning' && resp.data[item].length) {
                const content = `${this.$t(`profiling`)[item].desc}`;
                const divDom = `<div class="content-style">
                         <div class="content-icon el-icon-caret-right"></div>
                         <div class="helper-content-style">${content}</div>
                       </div>`;
                helperDiv.appendChild(new DOMParser().parseFromString(divDom, 'text/html').body.childNodes[0]);
     ...
     */
    backToDashboard() {
      let path = '/profiling-cpu/performance-dashboard';
      if (this.tabData.activeName === this.tabData.tabPanes[1].name) {
        path = '/profiling-cpu/resource-dashboard';
      }
      this.$router.push({
        path: path,
        query: {
      ...
      if (tabItem && tabItem.name) {
        ...
        switch (tabItem.name) {
          case this.tabData.tabPanes[0].name:
            path = '/profiling-cpu/performance-dashboard';
            break;
